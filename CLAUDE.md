...
This project serves two interconnected functions:

### 1. Mockup Generation

Build polished UI for teligant.com marketing screenshots. Focus on visual quality and realistic data.

### 2. V1.5 Exploration

Discover patterns, data structures, and architecture that could inform (or become) production V1.5.

**Key insight:** The shared data layer (`src/data/`) intentionally mirrors production Prisma schemas. Components should use tenant context patterns even with fake data — this makes eventual API integration seamless.

### Current Phase

- **Now:** Advanced mockup with production-ready patterns
- **Soon:** Add tenant context awareness (even with fake data)
- **Later:** Evaluate backend strategy and connect to real APIs incrementally

---

## Cross-Project Workflow

This mockup project works in tandem with **teligant-product** (the production V1 repositories). Two Claude Code instances collaborate via document handoffs.

### Document Flow

```
hedfirst-mockup (this project)          teligant-product
─────────────────────────────           ─────────────────
_docs/from-production/          ←───    Production Claude generates
  (context about V1)                    architecture/schema docs

_docs/to-production/            ───→    _docs/mockup-insights/
  (UI patterns, data needs)             Production Claude reads
```
...
| Folder                           | Purpose                                                |
| -------------------------------- | ------------------------------------------------------ |
| `_docs/from-production/`         | Documents FROM Production Claude providing V1 context  |
| `_docs/to-production/`           | Documents TO Production Claude sharing mockup insights |
| `_docs/architecture-decisions/`  | Internal ADRs for significant decisions                |
| `_docs/session-notes/`           | Session summaries for continuity                       |
| `_docs/_multi-tenant-work-plan/` | Production work plans (existing)                       |

### When to Generate Handoffs

| Trigger                           | Action                                            |
| --------------------------------- | ------------------------------------------------- |
| End of significant feature        | Generate session note + handoff if insights exist |
| Architecture decision made        | Document in `architecture-decisions/`             |
| Data shape requirement discovered | Include in handoff to production                  |
| Need production context           | Ask for document from teligant-product            |

### How to Request Handoffs

**To generate a handoff for production:**

> "Generate a handoff document for the production project. Focus on [specific topic]."

**To summarize a session:**

> "Summarize this session for future reference."

---

## Context Switching

When switching between mockup building and production strategy discussion:

### Switching TO Production Thinking

> "Pause on the UI. Let's discuss how [X] would work in production. Reference [specific doc] if needed."

### Switching BACK to Building

> "Good discussion. Back to building — let's implement [next feature]."

### Starting a New Session

> "Read `_docs/session-notes/[latest]` and `_docs/architecture-decisions/[relevant]` to get context."
