...
export default function PatientSidebarNav({
  ...
  // Check if current path matches item href (exact or starts with for nested routes)
  const isActive = (href: string) => {
    if (pathname === href) return true
    // For the base patient detail page, only match exactly
    if (href.endsWith("/")) return pathname === href.slice(0, -1)
    return false
  }
